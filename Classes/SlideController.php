...
class SlideController extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	protected static $templaVoilaFrontendController = NULL;


	/**
	 * Constructor takes care of generating a (static) tx_templavoila_pi1 instance for calling various
	 * support methods.
	 *
	 * @return void
	 */
	public function __construct() {
		if (!is_object(self::$templaVoilaFrontendController)) {
			self::$templaVoilaFrontendController = GeneralUtility::makeInstance('tx_templavoila_pi1');
			self::$templaVoilaFrontendController->inheritValueFromDefault = $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_templavoila_pi1.']['dontInheritValueFromDefault'] ? 0 : 1;
		}
	}
	...
	protected function getPageFlexValue($page, $field) {
		...
		$fieldPath = GeneralUtility::trimExplode(',', $field, 1);
		if (count($fieldPath) > 1) {
			throw new Exception('Handling repeatable sections for content sliding is currently not implemented properly. Contact extension author.');
		}
		$field_LP = $ds['ROOT']['el'][$fieldPath[0]]['tx_templavoila'];
		...
		do {
			...
			if (is_array($data = $xml['data']['sDEF'][$lKey])) {
				$value = $this->getSubKey($data, $fieldPath);
				if ($vKey !== 'vDEF') {
					$value = self::$templaVoilaFrontendController->inheritValue($value, $vKey, $field_LP['langOverlayMode']);
				} else {
					$value = $value['vDEF'];
				}
			}
	 ...
	 * @param string $vKey: The language value which should get returned (i.e. vDEF, vDE, vPT, etc.). If FALSE the whole array will get returned.
	 * @return string The contents of the field
	 */
	function getSubKey($arr, $keys, $vKey = FALSE) {
		if (!is_array($arr)) {
			return '';
		}
		if (!count($keys)) {
